...
export default function AdminHotels() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedHotel) {
        // Update
        const updated = await hotelService.updateHotel(selectedHotel.id, formData);
        setHotelsList(hotelsList.map(h => h.id === selectedHotel.id ? updated : h));
  ...
  if (isLoading) return <div className="text-center py-12">Chargement des hôtels...</div>;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {hotelsList.reduce((acc, h) => acc + (h.rooms?.length || 0), 0)}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">
                {hotelsList.length > 0 ? (hotelsList.reduce((acc, h) => acc + h.rating, 0) / hotelsList.length).toFixed(1) : '0.0'}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-[#C6A87C]">
                {Math.round(hotelsList.filter(h => h.pricePerNight).reduce((acc, h) => acc + (h.pricePerNight || 0), 0) / (hotelsList.filter(h => h.pricePerNight).length || 1)) || 0}
              </p>
        ...
        {hotelsList.map((hotel) => (
          <div key={hotel.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
            <div className="relative h-48 bg-gray-200">
              {hotel.image ? (
                <img 
                   src={hotel.image.startsWith('http') ? hotel.image : `${import.meta.env.VITE_API_URL}${hotel.image}`} 
                  alt={hotel.name}
            ...
            <div className="p-5">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-semibold text-gray-900">{hotel.rooms?.length || 0}</p>
                </div>
              </div>
